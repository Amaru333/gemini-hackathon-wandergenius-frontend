    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-500 text-sm">{isNewItinerary ? "Generating your personalized itinerary..." : "Loading trip..."}</p>
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        <div className="bg-red-50 rounded-xl p-10 border border-red-100">
          ...
          <Link to="/history" className="inline-flex items-center gap-2 bg-slate-900 text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-slate-800 transition-colors">
            Back to History
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <Link to="/history" className="flex items-center gap-1.5 text-slate-500 hover:text-slate-900 text-sm font-medium transition-colors">
      ...
      <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-8 text-white mb-8 relative group">
        ...
        <div className="relative">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-3">
            <h1 className="text-3xl font-bold">{trip.destinationName}</h1>
          ...
          {(trip.destinationLat && trip.destinationLng) || (generationData?.lat && generationData?.lng) ? (
            <div className="mt-4">
              <WeatherCard lat={trip.destinationLat || generationData?.lat} lng={trip.destinationLng || generationData?.lng} />
            </div>
          ) : null}
        </div>
      </div>
      ...
      <div className="flex border-b border-slate-200 mb-8">
        <button
          ...
          className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors ${
            activeTab === "itinerary" ? "border-slate-900 text-slate-900" : "border-transparent text-slate-500 hover:text-slate-700"
          }`}
        >
          Daily Plan
        </button>
        <button
          ...
          className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${
            activeTab === "photos" ? "border-rose-500 text-rose-600" : "border-transparent text-slate-500 hover:text-slate-700"
          }`}
        >
          <Camera className="w-4 h-4" />
          Photos
          {photoCount > 0 && <span className={`px-2 py-0.5 rounded-full text-xs ${activeTab === "photos" ? "bg-rose-100 text-rose-600" : "bg-slate-100 text-slate-600"}`}>{photoCount}</span>}
        </button>
        <button
          onClick={() => setActiveTab("budget")}
          className={`px-6 py-3 text-sm font-medium border-b-2 transition-colors ${
            activeTab === "budget" ? "border-indigo-600 text-indigo-600" : "border-transparent text-slate-500 hover:text-slate-700"
          }`}
        >
          Budget Tracker
        </button>
      ...
      {activeTab === "budget" ? (
        ...
        <div className="max-w-4xl">
          <PhotoJournal tripId={trip.id} days={trip.days} destinationName={trip.destinationName} onPhotoCountChange={setPhotoCount} />
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              {itinerary.map((day) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                    activeDay === day.day ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
                  }`}
                >
                  Day {day.day}
                </button>
                  ...
                  {currentDay.activities?.map((activity, i) => (
                    <div key={i} className="flex gap-4">
                      <div className="flex flex-col items-center">
                        <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center">{getTimeIcon(activity.time)}</div>
                        {i < currentDay.activities.length - 1 && <div className="w-px h-full bg-slate-200 my-2" />}
                      </div>
          ...
          <div>
            <PackingList tripId={trip.id} items={checklist} onItemToggle={toggleChecklistItem} onItemsUpdate={setChecklist} />
          </div>
              ...
              <div className="space-y-4">
                {reviews.reviews.map((review) => (
                  <div key={review.id} className="bg-white border border-slate-200 rounded-xl p-5">
                    ...
                    {review.comment && <p className="text-slate-600 italic">"{review.comment}"</p>}

                    <div className="flex gap-4 mt-3 pt-3 border-t border-slate-100 text-xs text-slate-500">
                      <span className="flex items-center gap-1">
                        <DollarSign className="w-3 h-3" /> {review.budgetRating}/5
                      </span>
                      <span className="flex items-center gap-1">
                        <MapPin className="w-3 h-3" /> {review.locationRating}/5
                      </span>
                      <span className="flex items-center gap-1">
                        <Sparkles className="w-3 h-3" /> {review.activitiesRating}/5
                      </span>
                    </div>
                  </div>
                ))}
      ...
      <InviteModal isOpen={isInviteModalOpen} onClose={() => setIsInviteModalOpen(false)} tripId={trip.id} isOwner={trip.isOwner ?? true} />
